...
export default function DriverDashboard() {
  ...
  async function acceptRide(id) {
    ...
    // Optimistically transition accepted ride to history
  ...
  // Smart polling for new pending rides
  useEffect(() => {
    let intervalId
    if (activeTab === 'pending') {
...
function PassengerCard({ data }) {
  if (!data?.name) return <div style={{ color: 'var(--text-muted)', fontSize: '0.8rem', marginTop: '0.8rem', textAlign: 'center' }}>Loading passenger details...</div>
  return (
    <div style={{ marginTop: '1rem', background: 'var(--bg-secondary)', borderRadius: 'var(--radius-sm)', padding: '1rem', border: '1px solid var(--border)', display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
      ...
      {data.vehicleNumber && (
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', fontSize: '0.82rem', color: 'var(--text-secondary)' }}>
          <User size={14} /> Roll No: {data.vehicleNumber}
        </div>
      )}
    </div>
  )
